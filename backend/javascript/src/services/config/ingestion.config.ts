...
export class IngestionConfig {
    ...
    get chatSimilarityThreshold(): number {
        return Number(this.configService.get<string>('CHAT_SIMILARITY_THRESHOLD')) || 0.5;
    }

    get chatRetrievalCount(): number {
        return Number(this.configService.get<string>('CHAT_RETRIEVAL_COUNT')) || 5;
    }
}